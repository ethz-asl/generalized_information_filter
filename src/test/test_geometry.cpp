...
TEST(Geometry, TestQuaternion) {
  ...
  Quaternion q(Quaternion::Identity().slerp(
      0.10, Quaternion::FromTwoVectors(
                Vector3(1, 0, 0), Vector3(0, 0, 1))));

  Quaternion p(Quaternion::Identity().slerp(
      0.20, Quaternion::FromTwoVectors(
                Vector3(1, 0, 0), Vector3(0, 0, 1))));
  ...
  EXPECT_TRUE(
      EIGEN_MATRIX_EQUAL_DOUBLE(theata, theta_new));

  // Check: (q boxplus theata) boxplus -theata = q
  Quaternion q_orig;
  Vector3 theta_neg = -theata;
  quaternion_helpers::boxPlus(
      q_rot, theta_neg, &q_orig);


  EXPECT_TRUE(
      EIGEN_MATRIX_EQUAL_DOUBLE(q.vec(), q_orig.vec()));
  ...
  EXPECT_TRUE(
        EIGEN_MATRIX_EQUAL_DOUBLE(p.vec(), p_orig.vec()));
}
