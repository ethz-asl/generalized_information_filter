...
TEST(Geometry, TestQuaternionSmallAngle) {
  // Initial state values.
  Quaternion q(Quaternion::Identity().slerp(
      0.10, Quaternion::FromTwoVectors(Vector3(1, 0, 0), Vector3(0, 0, 1))));

  Quaternion p = q; // REALLY small angle ;)

  Vector3 theta(0.2, 0.3, -0.1);
  theta = 0.1 * kQuaternionSmallAngleThreshold * theta.normalized();

  // Check: (q boxplus theta) boxminus q = theta
  Quaternion q_rot;
  Vector3 theta_new;
  quaternion_helpers::boxPlus(q, theta, &q_rot);
  quaternion_helpers::boxMinus(q_rot, q, &theta_new);

  EXPECT_TRUE(EIGEN_MATRIX_EQUAL_DOUBLE(theta, theta_new));

  // Check: (q boxplus theta) boxplus -theta = q
  Quaternion q_orig;
  Vector3 theta_neg = -theta;
  quaternion_helpers::boxPlus(q_rot, theta_neg, &q_orig);
