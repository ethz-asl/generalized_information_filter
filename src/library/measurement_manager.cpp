...
void MeasurementManager::addMeasurement(
  ...
  CHECK(timeline_key < timelines_.size())
      << "Timeline key does not exist. This probably means that the "
         "measurement is not used by any residual.";
...
void MeasurementManager::printTimeline() const {
  ...
  for (const Timeline& current_timeline : timelines_) {
    ...
    timeline_iterators_end.push_back(
        current_timeline.timed_measurements_.end());
...
bool MeasurementManager::splitMeasurements(
  ...
  if (measurement_start.first != timestamp_split_ns ||
      measurement_end.first != timestamp_split_ns) {
    measurement_split->first = timestamp_split_ns;
    measurement_split->second = measurement_start.second->split(
        *measurement_end.second, measurement_start.first, timestamp_split_ns,
        measurement_end.first);
    return true;
  }
  return false;
}

// This function returns a vector of measurements in the requested range. If
// measurements at the boarder don't exist they are interpolated and added to
// the memory_manager for deallocation.
// time      x   x   x   x   x   x   x
// min/max     ^               ^
// return      x x   x   x   x x
bool MeasurementManager::extractRelevantMeasurements(
  ...
  measurement_buffer =
      timeline.getMeasurementsInRange(timestamp_start_ns, timestamp_end_ns);
  ...
  if (splitMeasurements(
          measurement_buffer[0], measurement_buffer[1], timestamp_start_ns,
          &temp_measurement)) {
  ...
  if (splitMeasurements(
          measurement_buffer[last_idx - 1], measurement_buffer[last_idx],
          timestamp_end_ns, &temp_measurement)) {
...
bool MeasurementManager::updateStrategy(
  ...
  measurement_buffer->timelines[active_measurement_idx].emplace_back(
      measurement);

  // TODO(burrimi): should we also check the oldest timestamp of mergeable
  // residuals?

  // Check if all the mergeable measurements have a newer measurement to allow
  // for interpolation.
  for (size_t i = 0; i < timelines_.size(); ++i) {
    const Timeline& current_timeline = timelines_[i];
    if (current_timeline.isEmpty()) {
    ...
    extractRelevantMeasurements(
        ...
        &measurement_buffer->managed_measurements);
  }
