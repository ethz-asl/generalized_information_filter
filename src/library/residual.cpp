...
bool ResidualBase::inputTypesValid(const std::vector<BlockBase*>& state1, const std::vector<BlockBase*>& state2) const {
  CHECK(state1.size() == state1_block_types_.size()) << "Number of blocks for first state not correct!!!";
  std::vector<BlockType>::const_iterator current_state_type_it = state1_block_types_.begin();
